const class3_students = require('../models/3_Models');

const getAllStudent = async (req,res,next)=>
{ const Students = await class3_students.find();
res.status(200).json({ Students}) }

const getByid = async(req,res,next)=>
{const _id = req.params.id; const doc = await class3_students.find({_id})
res.send(doc);}

const deleteStudent = async(req,res,next)=>
{   const _id = req.params.id;
     const doc = await class3_students.deleteOne({_id});
